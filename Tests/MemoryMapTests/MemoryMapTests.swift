...
final class MemoryMapTests: XCTestCase {
  ...
  func testFilePermissions() throws {
    ...
    let readOnlyURL = FileManager.default.temporaryDirectory.appendingPathComponent(
      UUID().uuidString)
  ...
  func testConcurrentReadWrite() throws {
    ...
    let writeQueue = DispatchQueue(
      label: "com.memorymap.write", qos: .userInitiated, attributes: .concurrent)
    // Create a queue for reads
    let readQueue = DispatchQueue(
      label: "com.memorymap.read", qos: .userInitiated, attributes: .concurrent)
